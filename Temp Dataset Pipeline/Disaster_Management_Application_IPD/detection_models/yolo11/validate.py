...
def parse_args():
    parser = argparse.ArgumentParser(description="Validate YOLO11 on a dataset")
...
def main():
    ...
    print(f"\n--- Validating YOLO11 on {args.dataset} ---")
    ds_info = get_dataset(args.dataset, args.dataset_root)
    
    model = YOLO(args.weights)
    
    project_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "outputs", "yolo11_val")
