...
class TestS2Preprocessing(unittest.TestCase):
    ...
    def test_processing_and_scaling(self, mock_rasterio_open):
        ...
        b02_data = np.full((100, 100), 5000, dtype=np.uint16)
        b03_data = np.full((100, 100), 5000, dtype=np.uint16)
        ...
        b8a_data = np.full((50, 50), 6500, dtype=np.uint16)
        ...
        self.memfiles = {
            "mock_b02": create_synthetic_tiff(b02_data, transform_10m),
            "mock_b03": create_synthetic_tiff(b03_data, transform_10m),
            "mock_b04": create_synthetic_tiff(b04_data, transform_10m),
            "mock_b08": create_synthetic_tiff(b08_data, transform_10m),
            "mock_b8a": create_synthetic_tiff(b8a_data, transform_20m),
