...
def _download_tile(zoom, x, y, cache_dir):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return x, y, Image.open(io.BytesIO(resp.content)).convert('RGB')
        elif resp.status_code == 404:
            # Missing tile
            return x, y, None
    except Exception as e:
        logger.error(f"Failed to fetch high-res tile {x},{y}: {e}")
    return x, y, None
...
def fetch_high_res_basemap(bounds_wgs84, zoom=18, canonical_aoi: 'CanonicalAOI' = None):
    ...
    Downloads Esri World Imagery (approx 0.6m resolution)
    """
    min_lon, min_lat, max_lon, max_lat = bounds_wgs84
    cache_dir = "cache/imagery/esri"
    ...
    for attempt_zoom in [zoom, zoom - 1, zoom - 2]:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            for x in range(x_min, x_max + 1):
                for y in range(y_min, y_max + 1):
                    tasks.append(executor.submit(_download_tile, attempt_zoom, x, y, cache_dir))
        ...
        if missing > total_tiles * 0.5:
            logger.warning(f"Over 50% tiles missing at zoom {attempt_zoom}. Trying lower zoom...")
        ...
        if canonical_aoi:
            ...
            return {
                ...
                "metadata": {
                    "scene_id": f"Esri_World_Imagery_Z{attempt_zoom}_Aligned",
        ...
        return {
            ...
            "resolution": res_m,
            "metadata": {
                "scene_id": f"Esri_World_Imagery_Z{attempt_zoom}",
                "acquisition_datetime": datetime.now(timezone.utc).isoformat(),
                "crs": "EPSG:3857",
                "cloud_cover": 0.0
            }
        }
    raise Exception("Failed to retrieve Esri World Imagery at any viable zoom level.")
